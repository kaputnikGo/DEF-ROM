; DEF-ROM NMI-ORGAN SOUNDS CODE - 26 Mar 2021
...
0010 : 00 00                          ; X SYN8, A
0012 : 00 00                          ; write flood, should contain several 7E 01 72 writes for PC jmps
...
0105 : CE 01 A0   ldx #$01A0          ;load X with 01A0 (addr of melody data)
;PRM72
...
0111 : 4C         inca                ;incr A
0112 : 8D 26      bsr L013A           ;branch sub CALCOS (need a jsr 3 byte)
0114 : 01         nop                 ;
0115 : 20 F1      bra L0052           ;branch always PRM72
;PRM73
0117 : 08         inx                 ;incr X
0118 : DF 0B      stx X000B           ;store X in addr 0B
011A : 8D 1E      bsr L013A           ;branch sub CALCOS  (jsr 3 byte)
; need 1 byte instruction here
...
014D : 23 15      bls L011C           ;branch if lower or same SYN83 <-- write loop from 0012
014F : 81 03      cmpa  #$03          ;compare A with 03h (0000 0011)
0151 : 27 09      beq L0114           ;branch =0 SYN82 <-- loop countdown =0
0153 : C6 01      ldab  #$01          ;load B with 01h (0000 0001)
0155 : E7 00      stab  $00,x         ;store B in addr X + 00h <-- 01 nop writer
0157 : 08         inx                 ;incr X
...
0164 : C6 7E      ldab  #$7E          ;load B with 7Eh (1111 1110)
...
0199 : 09         dex                 ;decr X
019A : 27 03      beq  L019F          ;branch =0 SYN84
019C : 7E 00 12   jmp  L0012          ;jump to timer location 0012 (to jmp writes that set freq/pitch duration)
;SYN84
...
01B0 : 02 3E 7C 04 03 FF 3E 3F        ;X=02 3E (1st length), 01BF X=03 FF(2nd length), 7C 04 2nd note, 3F 
01B8 : 2C E2 7C 12 0D 74 7C 0D        ;X=2C E2(3rd length)
01CO : 0E 41 7C 23 0B 
;01C4 end
;*************************************;
;01C5 monitor RAM
