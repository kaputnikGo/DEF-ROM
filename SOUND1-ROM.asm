...
;SYN14 LF853 V1:
F853 : 09				  dex           ;decr X
F854 : 27 10		  beq	LF866     ;branch if Z=1 SYN16 (VSWEEP)
F856 : 4A				  deca          ;decr A
F857 : 26 FA		  bne	LF853     ;branch if Z=0 SYN14 (V1)
F859 : 73 04 00   com	$0400     ;complement 1s in DAC output SOUND
F85C : 96 19		  ldaa	$19     ;load A with value in addr 19 (HICNT) (HI CYCLE)
;SYN15 LF85E V2:
F85E : 09				  dex           ;decr X
F85F : 27 05		  beq	LF866     ;branch if Z=1 SYN16 (VSWEEP)
F861 : 4A				  deca          ;decr A
F862 : 26 FA		  bne	LF85E     ;branch if Z=0 SYN15 (V2)
F864 : 20 E8		  bra	LF84E     ;branch always SYN13 (V0LP) (LOOP BACK)
;SYN16 LF866 VSWEEP:
F866 : B6 04 00   ldaa	$0400   ;load A with value in addr DAC output SOUND
F869 : 2B 01		  bmi	LF86C     ;branch if N=1 SYN17 (VS1)
...
F892 : 20 0A		  bra	LF89E     ;branch always SYN21 (LITEN)
;*************************************;
;PARAM - Appear (was synth2)
;*************************************;
...
F89C : 20 00		  bra	LF89E     ;branch always SYN21 (LITEN)
;*************************************;
;SYNTH2 - Lightning+Appear Noise Routine
;*************************************;
;SYN21 LF89E LITEN:
...
F8B7 : 24 03		  bcc	LF8BC     ;branch if C=0 SYN24 (LITE2)
...
F8DA : 20 00		  bra	LF8DC     ;branch always SYN31 (NOISE)
;*************************************;
;SYNTH3 - White Noise Routine
;*************************************;
...
F8F4 : 24 02		  bcc	LF8F8     ;branch if C=0 SYN35 (NOISE2)
...
;SYN36 LF8FD NOISE3:
F8FD : 09				  dex           ;decr X
F8FE : 26 FD		  bne	LF8FD     ;branch if Z=0 SYN36 (NOISE3)
F900 : 5A				  decb          ;decr B (FIN CYCLE COUNT?)
F901 : 26 E1		  bne	LF8E4     ;branch if Z=0 SYN34 (NOISE1)(NO)
F903 : D6 10		  ldab	$10     ;load B with value in addr 10 (NAMP)(DECAY AMP)
F905 : D0 0F		  subb	$0F     ;B = B - value in addr 0F (DECAY) 
F907 : 27 09		  beq	LF912     ;branch if Z=1 SYN37 (NSEND)
...
F90E : 27 D0		  beq	LF8E0     ;branch if Z=1 SYN33 (NOIS00)(NO)
F910 : 20 CC		  bra	LF8DE     ;branch always SYN32 (NOISE0)
;SYN37 LF912 NSEND:
F912 : 39				  rts           ;return subroutine
;*************************************;
;PARAM3 - Background 1 Routine
;*************************************;
;LF913 BG1:
...
F91A : 20 14		  bra	LF930     ;branch always SYNTH4 (FNOISE)
;*************************************;
;PARAM4 - Thrust
;*************************************;
...
F921 : 20 0D		  bra	LF930     ;branch always SYNTH4 (FNOISE)
;*************************************;
;PARAM5 - Cannon
;*************************************;
...
F92E : 20 00		  bra	LF930     ;branch always SYNTH4 (FNOISE)
;*************************************;
;SYNTH4 - Filtered Noise Routine
;*************************************;
...
F950 : 27 02		  beq	LF954     ;branch if Z=1 SYN43 (FNOIS2)
...
F95D : 27 26		  beq	LF985     ;branch if Z=1 SYN47 (FNOIS6)
...
F966 : 25 16		  bcs	LF97E     ;branch if C=1 SYN46 (FNOIS5)
F968 : 91 06		  cmpa	$06     ;compare A with value in addr 06 (LO)
F96A : 23 F0		  bls	LF95C     ;branch if C=1 and Z=1 SYN44 (FNOIS3)
F96C : 20 10		  bra	LF97E     ;branch always SYN46 (FNOIS5)
;SYN45 LF96E FNOIS4:
F96E : 09				  dex           ;decr X (SLOPE DOWN)
F96F : 27 14		  beq	LF985     ;branch if Z=1 SYN47 (FNOIS6)
...
F978 : 25 04		  bcs	LF97E     ;branch if C=1 SYN46 (FNOIS5)
F97A : 91 06		  cmpa	$06     ;compare A with value in addr 06 (LO)    
F97C : 22 F0		  bhi	LF96E     ;branch if C=0 and Z=0 SYN45 (FNOIS4)
;SYN46 LF97E FNOIS5:
...
F987 : 27 B5		  beq	LF93E     ;branch if Z=1 SYN42 (FNOIS1)
...
F99F : 26 98		  bne	LF939     ;branch if Z=0 SYN41 (FNOIS0)
F9A1 : C1 07		  cmpb	#$07    ;compare B with value 07h
F9A3 : 26 94		  bne	LF939     ;branch if Z=0 SYN41 (FNOIS0)
F9A5 : 39				  rts           ;return subroutine
;*************************************;
;SYNTH5 - Radio
;*************************************;
F9A6 : 86 FD		  ldaa	#$FD    ;load A with value FDh (#RADSND/$100 SOUND TABLE)
...
F9B9 : 25 04		  bcs	LF9BF     ;branch if C=1 SYN53 (RADIO2)
F9BB : 20 00		  bra	LF9BD     ;branch always (*+2)(EQUALIZE TIME)
F9BD : 20 03		  bra	LF9C2     ;branch always SYN54 (RADIO3)
;SYN53 LF9BF RADIO2:
F9BF : 08				  inx           ;incr X (CARRY?, RAISE FREQ)
F9C0 : 27 11		  beq	LF9D3     ;branch if Z=1 SYN55 (RADIO4)(DONE?)
;SYN54 LF9C2 RADIO3:
F9C2 : DF 07		  stx	$07       ;store X in addr 07 (TEMPX)
F9C4 : 84 0F		  anda	#$0F    ;and A with value 0Fh (SET POINTER)
F9C6 : 8B 84		  adda	#$84    ;add A with value 84h (RADSND!.$FF)
...
F9D1 : 20 DC		  bra	LF9AF     ;branch always SYN51 (RADIO1)
;SYN55 LF9D3 RADIO4:
...
F9F0 : 2A E8		  bpl	LF9DA     ;branch if N=0 SYN61 (HYPER1)(DONE?)
...
F9FC : 26 F8		  bne	LF9F6     ;branch if Z=0 SYN71 (SCREM1)
...
FA10 : 2A 02		  bpl	LFA14     ;branch if N=0 SYN74 (SCREM4)(ADD AMPLITUDE IF MINUS)
...
FA2C : 27 0B		  beq	LFA39     ;branch if Z=1 SYN77 (SCREM7)
FA2E : 81 37		  cmpa	#$37    ;compare A with value 37h
FA30 : 26 04		  bne	LFA36     ;branch if Z=0 SYN76 (SCREM6)
...
FA3E : 26 EA		  bne	LFA2A     ;branch if Z=0 SYN75 (SCREM5)
FA40 : 5D				  tstb          ;test B=0 (DONE?)
FA41 : 26 BF		  bne	LFA02     ;branch if Z=0 SYN72 (SCREM2)
...
FA52 : CE FD 94   ldx	#$FD94    ;load X with FD94 (ORGTAB)(wavetable)
;PRM72 LFA55 ORGNT2:
FA55 : A6 00		  ldaa	$00,x   ;load A with addr X + 00h (TUNE TABLE LENGTH)
FA57 : 27 2D		  beq	LFA86     ;branch Z=1 PRM75 (ORGNT5) (INVALID TUNE)
FA59 : 7A 00 0D   dec	$000D     ;decr addr 0D (TEMPA)
FA5C : 27 06		  beq	LFA64     ;branch Z=1 PRM73 (ORGNT3)
FA5E : 4C         inca          ;incr A
FA5F : BD FC F0   jsr	LFCF0     ;jump sub CALCOS
FA62 : 20 F1		  bra	LFA55     ;branch always PRM72 (ORGNT2)
;PRM73 LFA64 ORGNT3:
FA64 : 08         inx           ;incr X
...
;LFAB2 SYN83 ORGAN1 stores 7E FAB2 jmp $FAB2
...
FAD0 : 1B				  aba           ;A = A + B <-- not in Pharoah ROM
...
FAD9 : 09				  dex           ;decr X
FADA : 27 03		  beq	LFADF     ;branch Z=1 SYN84 (NOTE OVER?)
...
;*************************************;
;PARAM10 - Background End Routine
;*************************************;
...
;*************************************;
;PARAM11 - Background Sound #2 increment
;*************************************;
;BG2INC
...
FB03 : 26 01		  bne	LFB06     ;branch if Z=0 P111 (BG2IO)
...
;*************************************;
;UTIL2 - Background 2 Routine
;*************************************;
;BG2
FB0A : 86 0E		  ldaa	#$0E    ;load A with value 0Eh (#(TRBV-SVTAB)/7) (GET SOUND)
FB0C : BD FB 57   jsr	LFB57     ;jump sub PRM132 (GWLD)
...
FB14 : BD FC 0F   jsr	LFC0F     ;jump sub PRM142 (GEND60)
;UTL1 LFB17 BG2LP:
FB17 : 7C 00 13   inc	$0013     ;incr value in addr 0013 (GDCNT)
FB1A : BD FC 11   jsr	LFC11     ;jump sub PRM143 (GEND61)
FB1D : 20 F8		  bra	LFB17     ;branch always UTL1 (BG2LP)
;*************************************;
;PARAM12 - Spinner #1 Sound
;*************************************;
;SP1
FB1F : 86 03		  ldaa	#$03    ;load A with value 03h (#(CABSHK-VVECT)/9)
FB21 : BD F8 2A   jsr	LF82A     ;jump sub PARAM1 (VARILD)
FB24 : D6 02		  ldab	$02     ;load B with value at addr 02 (SP1FLG)
FB26 : C1 1F		  cmpb	#$1F    ;compare B with value 01h (#SP1MAX-1)
FB28 : 26 01		  bne	LFB2B     ;branch if Z=0 PRM121 (SP1A)
...
FB38 : 4A				  deca          ;decr A
FB39 : 20 F7		  bra	LFB32     ;branch always PRM122 (SP11)
;PRM123 LFB3B:
FB3B : CB 05		  addb	#$05    ;add B with 05h
FB3D : 4A				  deca          ;decr A
FB3E : 26 FB		  bne	LFB3B     ;branch if Z=0 PRM123 (SP12)
FB40 : D7 0F		  stab	$0F     ;store B in addr 0F (LOPER)
;PRM124 LFB42:
FB42 : BD F8 3F   jsr	LF83F     ;jump sub SYNTH1 (VARI) (DO IT)
FB45 : 20 FB		  bra	LFB42     ;branch always PRM124 (SP1LP)
;*************************************;
;PARAM13 - Laser Ball Bonus #2
;*************************************;
;BON2
FB47 : 96 03		  ldaa	$03     ;load A with value at addr 03 (B2FLG)
FB49 : 26 09		  bne	LFB54     ;branch if Z=0 PRM131 (BON21)
FB4B : 7C 00 03   inc	$0003     ;incr value at addr 0003 (B2FLG)
FB4E : 86 0D		  ldaa	#$0D    ;load A with value 0Dh (#(BONV-SVTAB)/7)
FB50 : 8D 05		  bsr	LFB57     ;branch sub PRM132 (GWLD)
FB52 : 20 69		  bra	LFBBD     ;branch always SYNTH9 (GWAVE)
;PRM131 LFB54 BON21:
FB54 : 7E FC 04   jmp	LFC04     ;jump to PARAM14 PRM141 (GEND50)
;*************************************;
;PARAM13X - GWAVE Loader
;*************************************;
;PRM132 LFB57 GWLD:
...
FB5A : 1B				  aba           ;A = A + B
FB5B : 1B				  aba           ;A = A + B
FB5C : CE FE D6   ldx	#$FED6    ;load X with FED6h (wavetable fdb - SVTAB)(SOUND VECTOR TABLE)
...
FB7E : CE FE 37   ldx	#$FE37    ;load X with FE37h (wavetable fdb - GWVTAB)(CALC WAVEFORM ADDR)
;PRM133 LFB81 GWLD2:
...
FB8C : 20 F3		  bra	LFB81     ;branch always PRM133 (GWLD2)
;PRM134 LFB8E GWLD3:
FB8E : DF 14		  stx	$14       ;store X in addr 14 (GWFRM)
FB90 : BD FC 4B   jsr	LFC4B     ;jump sub PARAM18 (WVTRAN)(XSFER WAVE TO RAM)
...
FB99 : BD FC 5D   jsr	LFC5D     ;jump sub PARAM19 (WVDECA) (DECAY IT)
...
FBAB : CE FF 3F   ldx	#$FF3F    ;load X with FF3Fh (wavetable)(#GFRTAB)
...
FBCF : 27 26		  beq	LFBF7     ;branch if Z=1 PARAM14 (GEND)(FINISH ON ZERO)
...
FBDC : 26 FD		  bne	LFBDB     ;branch if Z=0 SYN95 (GPRLP)
...
FBE6 : 26 F1 		  bne	LFBD9     ;branch if Z=0 SYN94 (GOUTLP)
FBE8 : 5A				  decb          ;decr B
FBE9 : 27 DA		  beq	LFBC5     ;branch if Z=1 SYN92 (GPLAY)
...
FC19 : 2B 06		  bmi	LFC21     ;branch if N=1 PARAM15 (GW1)(DEC)
FC1B : AB 00		  adda	$00,x   ;add A with value at addr X + 00h (INC)
FC1D : 25 08		  bcs	LFC27     ;branch if C=1 PARAM15 PRM151 (GW2)(CARRY=OVERFLOW)
FC1F : 20 0B		  bra	LFC2C     ;branch always PARAM16 (GW2A)
;*************************************;
;PARAM15 (part of GWAVE)
;*************************************;
;GW1
FC21 : AB 00		  adda	$00,x   ;add A with value at addr X + 00h (DEC)
FC23 : 27 02		  beq	LFC27     ;branch if Z=1 PRM151 (GW2)(OVERFLOW ON EQ)
FC25 : 25 05		  bcs	LFC2C     ;branch if C=1 PARAM16 (GW2A)(OVERFLOW IF CARRY CLEAR)
;PRM151 LFC27 GW2:
FC27 : 5D		      tstb          ;test B and set N,Z (FOUND START YET?)
FC28 : 27 08		  beq	LFC32     ;branch if Z=1 PARAM16 PRM161 (GW2B)(NO)
FC2A : 20 0F		  bra	LFC3B     ;branch always PARAM17 (GW3)(YES, THIS IS THE END)
;*************************************;
;PARAM16 (part of GWAVE)
;*************************************;
;GW2A
FC2C : 5D		      tstb          ;test B and set N,Z
FC2D : 26 03		  bne	LFC32     ;branch if Z=0 PRM161 (GW2B)(ALREADY FOUND START)
FC2F : DF 17		  stx	$17       ;store X in addr 17 (GWFRQ)(FOUND START)
FC31 : 5C		      incb          ;incr B
;PRM161 LFC32 GW2B:
FC32 : 08		      inx           ;incr X
FC33 : 9C 19		  cpx	$19       ;compare X with value at addr 19 (FRQEND)
FC35 : 26 DD		  bne	LFC14     ;branch if Z=0 PARAM14 PRM144 (GW0)(NOT OVER YET)
FC37 : 5D		      tstb          ;test B and set N,Z (FOUND START?)
FC38 : 26 01		  bne	LFC3B     ;branch if Z=0 PARAM17 (GW3)(YES)
FC3A : 39				  rts           ;return subroutine (ALL OVER)
;*************************************;
;PARAM17 (part of GWAVE)
;*************************************;
;GW3
FC3B : DF 19		  stx	$19       ;store X in addr 19 (FRQEND)
FC3D : 96 11		  ldaa	$11     ;load A with value at addr 11 (GECDEC)(RE:XSFER WAVE?)
FC3F : 27 06		  beq	LFC47     ;branch if Z=1 PRM171 (GEND0)(NO)
FC41 : 8D 08		  bsr	LFC4B     ;branch sub PARAM18 (WVTRAN)(XSFER WAVE)
FC43 : 96 16		  ldaa	$16     ;load A with value at addr 16 (PRDECA)
FC45 : 8D 16		  bsr	LFC5D     ;branch sub PARAM19 (WVDECA)
;PRM171 LFC47 GEND0:
FC47 : 7E FB BD   jmp	LFBBD     ;jump SYNTH9 (GWAVE)
;LFC4A GEND1:
FC4A : 39				  rts           ;return subroutine, called by PARAM14 (TERMINATE)
;*************************************;
;PARAM18 called by PARAM13 - Wave Transfer Routine
;*************************************;
;WVTRAN
...
FC54 : 08				  inx           ;incr X
FC55 : BD FA E0   jsr	LFAE0     ;jump sub UTIL1 (TRANS)
...
;*************************************;
;PARAM19 called by PARAM13,PARAM14 - Wave Decay Routinue
;*************************************;
;decay amount in ACCA 1/16 per decay
;
;WVDECA
FC5D : 4D				  tsta          ;test A and set N,Z
FC5E : 27 2B		  beq	LFC8B     ;branch if Z=1 PRM191 (WVDCX)(NODECAY)
...
;IRQ4 LFCBB:
FCBB : BD FB 57   jsr	LFB57     ;jump sub PARAM13 PRM132 (GWLD)(GWAVE SOUNDS)
FCBE : BD FB BD   jsr	LFBBD     ;jump sub SYNTH9 (GWAVE)
FCC1 : 20 1A		  bra	LFCDD     ;branch always IRQ7 (IRQ3)
;IRQ5 LFCC3 IRQ10:
FCC3 : 81 1B		  cmpa	#$1B    ;compare A with value 1Bh (SPECIAL SOUND?)
FCC5 : 22 0E		  bhi	LFCD5     ;branch if C and Z=0 IRQ6 (IRQ20)(VARI SOIUND)
...
FCD3 : 20 08		  bra	LFCDD     ;branch always IRQ7 (IRQ3)
;IRQ6 LFCD5 IRQ20:
FCD5 : 80 1C		  suba	#$1C    ;A = A - 1Ch
FCD7 : BD F8 2A   jsr	LF82A     ;jump sub PARAM1 (VARILD)
FCDA : BD F8 3F   jsr	LF83F     ;jump sub SYNTH1 (VARI)
;IRQ7 LFCDD IRQ3:
FCDD : 96 00		  ldaa	$00     ;load A with value at addr 00 (BG1FLG)(BGROUND ACTIVE)
FCDF : 9A 01		  oraa	$01     ;OR A with value at addr 01 (B2FLG)
;IRQ8 LFCE1:
FCE1 : 27 FE		  beq	LFCE1     ;branch if Z=1 IRQ8 (NOPE)
...
FCE8 : 27 03		  beq	LFCED     ;branch if Z=1 IRQ9 (IRQXX)
FCEA : 7E F9 13   jmp	LF913     ;jump PARAM3 (BG1)
;IRQ9 LFCED IRQXX:
FCED : 7E FB 0A   jmp	LFB0A     ;jump UTIL2 (BG2)
;*************************************;
...
FCF6 : 24 03		  bcc	LFCFB     ;branch if C=0 CAL1 (ADDX1)
...
;*************************************;
;NMI - Diagnostic Processing Here
;*************************************;
...
FD30 : BD F8 2A   jsr	LF82A     ;jump sub PARAM1 (VARILD)
FD33 : BD F8 3F   jsr	LF83F     ;jump sub SYNTH1 (VARI)
...
;*************************************;
;VWTAB - Special Routine Jump Table
;*************************************;
;JMPTBL
FD42 : FB 1F                          ;PARAM12 (SP1)
FD44 : F9 13                          ;PARAM3  (BG1)
FD46 : FA FA                          ;PARAM11 (BG2INC)
FD48 : F8	8C                          ;PARAM2  (LITE)
FD4A : FB 47                          ;PARAM13 (BON2)
FD4C : FA F4                          ;PARAM10 (BGEND)
FD4E : F8	CD                          ;SYNTH3  (TURBO)
FD50 : F8 94                          ;SYNTH2  (APPEAR)
FD52 : F9 1C                          ;PARAM4  (THRUST)
FD54 : F9 23                          ;PARAM5  (CANNON)
FD56 : F9 A6                          ;SYNTH5  (RADIO)
FD58 : F9 D4                          ;SYNTH6  (HYPER)
FD5A : F9	F3                          ;SYNTH7  (SCREAM)
FD5C : FA 44                          ;PARAM6  (ORGANT)
FD5E : FA 87                          ;PARAM8	 (ORGANN)
;*************************************;
;VARI VECTORS
;*************************************;
;VVECT EQU *
...
FE52 : 7F 4E 24 09 00 09 24 4E        ;GS1 cont
;127,78,36,9,0,9,36,78
FE5A : 10 7F C5 EC E7 BF 8D 6D 6A 7F  ;GS12
;16,127,197,236,231,191,141,109,106
FE65 : 7F 94 92 71 40 17 12 39        ;GS12 cont
;127,148,146,113,64,23,18,57
FE6B : 10 FF FF FF FF 00 00 00 00     ;GSQ22
;16,$FF,$FF,$FF,$FF,0,0,0,0
FE74 : FF FF FF FF 00 00 00 00        ;GSQ22 cont
;$FF,$FF,$FF,$FF,0,0,0,0
