        ...
        ;
        ; similar to Barracora ROM, diff is Speech ROMs used in this game for some sound selects
...
F823 : 09         dex           ;decr X
F824 : 26 FB      bne  LF821     ;branch if Z=0 RSET1 (loop clears mem addr 007F down to 0000)
...
F82E : 48         asla          ;arith shift left A (bit0 is 0)
F82F : 48         asla          ;arith shift left A (bit0 is 0)
F830 : 48         asla          ;arith shift left A (bit0 is 0)
F831 : 1B         aba           ;A = A + B 
...
F83A : BD FB 8F   jsr  LFB8F     ;jump sub ADDX CALCOS
...
;SYN14
F852 : 09          dex           ;decr X
F853 : 27 10      beq  LF865     ;branch if Z=1 SYN16
F855 : 4A          deca          ;decr A
F856 : 26 FA      bne  LF852     ;branch if Z=0 SYN14
F858 : 96 2A      ldaa  $2A     ;load A with value at addr 2A
F85A : 73 04 00   com  $0400     ;complement 1s in DAC output SOUND
;SYN15
F85D : 09          dex           ;decr X
F85E : 27 05      beq  LF865     ;branch if Z=1 SYN16
F860 : 4A          deca          ;decr A
F861 : 26 FA      bne  LF85D     ;branch if Z=0 SYN15
F863 : 20 E8      bra  LF84D     ;branch always SYN13
;SYN16
F865 : B6 04 00   ldaa  $0400   ;load A with value at DAC output SOUND
F868 : 2B 01      bmi  LF86B     ;branch if N=1 SYN17
...
F89F : 27 2D      beq  LF8CE     ;branch Z=1 PRM27
F8A1 : 7A 00 1E   dec  $001E     ;decr value in addr 001E
F8A4 : 27 06      beq  LF8AC     ;branch Z=1 PRM25
F8A6 : 4C          inca          ;incr A
F8A7 : BD FB 8F   jsr  LFB8F     ;jump sub CALCOS
F8AA : 20 F1      bra  LF89D     ;branch always PRM24
;ORGNT3 PRM25
F8AC : 08          inx           ;incr X
F8AD : DF 1C      stx  $1C       ;store X in addr 1C
F8AF : BD FB 8F   jsr  LFB8F     ;jump sub CALCOS
...
F8CC : 26 E8      bne  LF8B6     ;branch Z=0 PRM26
;PRM27
...
;SYN24 - SYN23 stores 7E F8 FB )jmp $F8FB) within flood mem to here
...
F921 : 09          dex           ;decr X
F922 : 27 03      beq  LF927     ;branch Z=1 SYN25
...
F936 : 08          inx           ;incr X
F937 : 5A          decb          ;decr B
F938 : 26 EF      bne  LF929     ;branch Z=0 PRM31
...
F9B3 : BD FB 8F   jsr  LFB8F     ;jump sub CALCOS
...
F9BB : 97 21      staa  $21     ;sotre A in addr 21
...
F9DC : 4C          inca          ;incr A
F9DD : BD FB 8F   jsr  LFB8F     ;jump sub CALCOS
...
FA23 : 27 26      beq  LFA4B     ;branch if Z=1 PARAM5B
...
;SYN45
FA2F : 4A          deca          ;decr A
FA30 : 26 FD      bne  LFA2F     ;branch if Z=0 SYN45
...
FA3A : 26 F1      bne  LFA2D     ;branch if Z=0 SYN44
FA3C : 5A         decb          ;decr B
FA3D : 27 DA      beq  LFA19     ;branch if Z=1 SYN42
FA3F : 08         inx           ;incr X
FA40 : 09          dex           ;decr X
FA41 : 08          inx           ;incr X
FA42 : 09          dex           ;decr X
...
FA47 : 01          nop           ;no op
FA48 : 01          nop           ;2 cycles (38, all is timer?)
FA49 : 20 DF      bra  LFA2A     ;branch always SYN43
;
FA4B : 96 22      ldaa  $22     ;load A with value in addr 22
FA4D : 8D 64      bsr  LFAB3     ;branch sub PARAM8
FA4F : 7A 00 2F   dec  X002F     ;decr value in addr 002F
FA52 : 26 C1      bne  LFA15     ;branch if Z=0 SYN41
FA54 : 96 13      ldaa  $13     ;load A with value in addr 13
FA56 : 9A 14      oraa  $14     ;OR A with value in addr 14
FA58 : 26 46      bne  LFAA0     ;branch if Z=0 PARAM6 rts (?)
FA5A : 96 23      ldaa  $23     ;load A with value in addr 23
FA5C : 27 42      beq  LFAA0     ;branch if Z=1 PARAM6 rts (?)
FA5E : 7A 00 24   dec  X0024     ;decr value in addr 0024
FA61 : 27 3D      beq  LFAA0     ;branch if Z=1 PARAM6 rts (?) 
...
FA6F : 2B 06      bmi  LFA77     ;branch if N=1 PRM5B4
FA71 : AB 00      adda  $00,x   ;add A with value in addr X + 00h
FA73 : 25 08      bcs  LFA7D     ;branch if C=1 PRM5B5
FA75 : 20 0B      bra  LFA82     ;branch always PRM5B6
;PRM5B4
FA77 : AB 00      adda  $00,x   ;add A with value in addr X + 00h
FA79 : 27 02      beq  LFA7D     ;branch if Z=1 PRM5B5
FA7B : 25 05      bcs  LFA82     ;branch if C=1 PRM5B6
;PRM5B5
FA7D : 5D          tstb          ;test B (N=0(MSB), Z=1)
FA7E : 27 08      beq  LFA88     ;branch if Z=1 PRM5B7
FA80 : 20 0F      bra  LFA91     ;branch always PARAM6     
;PRM5B6
FA82 : 5D          tstb          ;test B (N=0(MSB), Z=1)
FA83 : 26 03      bne  LFA88     ;branch if Z=0 PRM5B7
FA85 : DF 28      stx  $28       ;store X in addr 28
FA87 : 5C          incb          ;incr B
;PRM5B7
FA88 : 08          inx           ;incr X
FA89 : 9C 2A      cpx  $2A       ;compare X with value in 2A
FA8B : 26 DD      bne  LFA6A     ;branch if Z=0 PRM5B3
FA8D : 5D          tstb          ;test B (N=0(MSB), Z=1)
FA8E : 26 01      bne  LFA91     ;branch if Z=0 PARAM6
FA90 : 39          rts           ;return subroutine
;
FA91 : DF 2A      stx  $2A       ;store X in addr 2A
FA93 : 96 22      ldaa  $22     ;load A with value in addr 22
FA95 : 27 06      beq  LFA9D     ;branch if Z=1 PRM61
FA97 : 8D 08      bsr  LFAA1     ;branch sub PARAM7
FA99 : 96 27      ldaa  $27     ;load A with value in addr 27
FA9B : 8D 16      bsr  LFAB3     ;branch sub PARAM8
;PRM61
...
;IRQ3 - jsr Speech ROM IC4 (paired with 5?)
...
;IRQ4 - jsr Speech ROM 6 (paired with 7?)
...
;*************************************;
;SYNTH5 (same as ROM1 SYNTH10)
;*************************************; 
FB9E : CE 00 E0   ldx  #$00E0         ;load X with 00E0h
;SYN51
FBA1 : 86 20      ldaa  #$20          ;load A with value 20h (0010 0000)
FBA3 : 8D EA      bsr  LFB8F          ;branch sub ADDX CALCOS
;SYN52
FBA5 : 09         dex                 ;decr X
FBA6 : 26 FD      bne  LFBA5          ;branch Z=0 SYN52
FBA8 : 7F 04 00   clr  $0400          ;clear (00) in DAC output SOUND
;SYN53
FBAB : 5A         decb                ;decr B
FBAC : 26 FD      bne  LFBAB          ;branch Z=0 SYN53
...
FBB6 : 26 E9      bne  LFBA1          ;branch Z=0 SYN51
...
FBCD : 3E         wai                 ;wait interrupt, CCodes to Stack, PC+1 and halt
;NMI2
FBCE : 7F 04 02   clr  $0402          ;clear addr 0402 PIA(NMI waits here)
...
FC00 : F6 DF FA   ldab  $DFFA         ;load B with value DFFAh - Speech ROM IC6(secondary speech rom addr)
FC03 : C1 7E      cmpb  #$7E          ;compare B with value 7Eh (looking for a jmp opcode)
FC05 : 26 05      bne  LFC0C          ;branch if Z=0 NMI5 (not present, check for main speech rom)
FC07 : BD DF FA   jsr  LDFFA          ;jump sub DFFA - Speech ROM IC6
FC0A : 20 AD      bra  LFBB9          ;branch always NMI
;NMI5
FC0C : F6 EF FA   ldab  XEFFA         ;load B with value EFFAh - Speech ROM IC4(main speech rom addr)
...
FC95 : 08                             ;
FC96 : 7F D9 
FC9A : 7F 24 00 24 08 FF FF FF
FCA2 : FF 00 00 00 00 08 00 40
FCAA : 80 00 FF 00 80 40 10 7F 
FCB2 : B0 D9 F5 FF F5 D9 B0 7F 
FCBA : 4E 24 09 00 09 24 4E 10 
FCC2 : FF FF FF FF FF FF FF FF 
FCCA : 00 00 00 00 00 00 00 00  
FCD2 : 10 00 F4 00 E8 00 DC 00  
FCDA : E2 00 DC 00 E8 00 F4 00 
FCE2 : 00 10 59 7B 98 AC B3 AC 
FCEA : 98 7B 59 37 19 06 00 06  
FCF2 : 19 37 18 FF FF FF BF FF 
FCFA : FF FF 7F FF FF FF 3F FF 
FD02 : FF FF 00 FF FF FF 3F FF 
FD0A : FF FF BF 30 83 78 69 5B  
FD12 : 4E 42 37 2D 24 1C 15 0F
FD1A : 0A 06 03 01 00 01 03 06  
FD22 : 0A 0F 15 1C 24 2D 37 42 
FD2A : 4E 5B 69 78 88 99 AB BE
FD32 : D2 E7 FD E7 D2 BE AB 99  
FD3A : 95 90 8C 88 47 8A 95 A0 
FD42 : AB B5 BF C8 D1 DA E1 E8
FD4A : EE F3 F7 FB FD FE FF FE  
FD52 : FD FB F7 F3 EE E8 E1 DA
FD5A : D1 C8 BF B5 AB A0 95 8A 
FD62 : 7F 75 6A 5F 54 4A 40 37
FD6A : 2E 25 1E 17 11 0C 08 04 
FD72 : 02 01 00 01 02 04 08 0C
FD7A : 11 17 1E 25 2E 37 40 4A  
FD82 : 54 5F 6A 75 7F 
;*************************************;
